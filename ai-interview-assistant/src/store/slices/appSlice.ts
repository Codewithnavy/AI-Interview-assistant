...
import { AppState, Candidate, InterviewSession, Question } from '../../types';
...
const appSlice = createSlice({
  ...
  reducers: {
    addCandidate: (state, action: PayloadAction<Omit<Candidate, 'id' | 'createdAt' | 'completedSessions' | 'profileComplete'>>) => {
      ...
      // Set the new candidate as current if no current candidate is set
    ...
    setActiveTab: (state, action: PayloadAction<'interviewee' | 'interviewer' | 'analytics'>) => {
